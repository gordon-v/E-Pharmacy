...
if ($error === false) {
    ...
    $sql = "INSERT INTO products 
        (product_catag, product_title, product_price, discounted_price, product_desc, product_date, product_img, product_left, product_author, image_1) 
        VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)";

    $stmt = $conn->prepare($sql);

// If `image_1` is the same as `product_img`, use `$file_name`. Otherwise, handle another file upload.
    $image_1 = $file_name;

    $stmt->bind_param(
        "ssddsssdds",  // Added one more "s" for `image_1`
        $_POST['prod-category'],
        $_POST['prod-title'],
        $_POST['prod-price'],
        $_POST['prod-discount'],
